...
//      cheap block search on a downsampled luma buffer â not full
...
//   4. Input and game simulation are never touched â this hook only
...
//     back rather than show a wrong guess â Low Latency falls back to
...
// same frame it was rendered â left as a follow-up if profiling shows
// it's needed.
...
// Tuning starting points â not measured on real hardware, expect to
// adjust after real testing.
constexpr PresetParams kPresets[3] = {
    {3, 12.0f},  // 0: Low Latency â small search, bails out readily
    {5, 22.0f},  // 1: Balanced
    {8, 40.0f},  // 2: Smoothness â bigger search, tries harder
};
...
// motion search. Single bilinear tap per output texel â a coarse proxy,
...
// first, then the GLES3 core name â Android's libGLESv2.so serves both
...
// game's own vertex attribute configuration â a very plausible cause of
...
bool hasEglExtension(EGLDisplay display, const char *name) {
  ...
  while ((match = std::strstr(match, name)) != nullptr) {
    const bool startsAtToken = match == extensions || match[-1] == ' ';
    const char terminator = match[nameLength];
    if (startsAtToken && (terminator == ' ' || terminator == '\0')) {
      return true;
    }
    match += nameLength;
  }
...
private:
  ...
  EGLBoolean handleSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    // Do not draw synthetic frames if either required platform feature is missing.
    // The former cross-fade fallback replaced the real frame and could ghost/flash.
    if (!mVaoSupported || !mTrueFrameGenAvailable ||
  ...
  // On a poor match (best SAD above threshold), returns (0,0) â callers
  // then blend without motion compensation (or, for Low Latency preset,
  // the caller instead snaps to a duplicated real frame â handled by
  ...
  void estimateMotion(int radius, float sadThreshold, float &mvU,
    ...
    if (bestSad < 0.0f || bestSad > sadThreshold) {
      return; // no confident match â caller falls back gracefully
    }
  ...
  float computeSad(int dx, int dy) const {
    ...
    if (count == 0) {
      return 1e9f; // no overlap at this shift â treat as a very poor match
    }
