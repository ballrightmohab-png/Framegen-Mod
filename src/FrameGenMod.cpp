// FrameGenMod.cpp
//
// A LeviLaunchroid native mod that implements basic "frame generation":
// it hooks eglSwapBuffers, keeps the last two real rendered frames as GL
// textures, and inserts a blended (cross-faded) frame between them before
// presenting the real one. This roughly doubles the number of frames
// actually presented to the display, at the cost of:
//   - ~1 extra frame of latency for the "real" frame (same tradeoff every
//     frame-generation technique makes, including DLSS 3 / FSR 3),
//   - visible ghosting on fast motion, since this is a plain temporal
//     blend and NOT motion-vector based interpolation.
//
  ...
  bool enable() {
    ...
    const bool moduleRegistered =
        pl::modmenu::ModuleBuilder(kModuleId, "Frame Generator")
            .modId(getSelf().getId())
            .description(
                "Doubles displayed frames by blending consecutive real "
                "frames. Adds ~1 frame of latency and some ghosting on "
                "fast motion; this is temporal blending, not motion-vector "
                "based interpolation.")
            .defaultEnabled(true)
...
private:
  ...
  EGLBoolean handleSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    if (mHaveHistory) {
      ...
      drawBlended(mix);
      const EGLBoolean firstResult = mOriginalSwapBuffers(dpy, surface);

      drawBlended(1.0f); // restore the true current frame
      const EGLBoolean secondResult = mOriginalSwapBuffers(dpy, surface);

      std::swap(mTexPrev, mTexCur);
      return (firstResult == EGL_TRUE && secondResult == EGL_TRUE)
                 ? EGL_TRUE
                 : EGL_FALSE;
    }
