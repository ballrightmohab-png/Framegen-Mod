// FrameGenMod.cpp
//
// A LeviLaunchroid native mod that implements real frame generation where
// the device supports it: it hooks eglSwapBuffers, keeps the last two
// rendered frames as GL textures, and inserts a blended frame between them,
// explicitly timestamped via the EGL_ANDROID_presentation_time extension
// (eglPresentationTimeANDROID) so the compositor knows exactly when each
// buffer is meant to appear rather than racing two swaps through back to
// back. On devices without that extension, it automatically falls back to
// a single-swap blend-only smoothing mode instead (no frame doubling, but
// no risk of the flicker an un-timed double swap causes).
//
...
// eglPresentationTimeANDROID is an extension (EGL_ANDROID_presentation_time)
// resolved at runtime via eglGetProcAddress, so we declare its signature
// ourselves rather than depending on a specific eglext.h being present.
...
public:
  ...
  bool enable() {
    ...
    // Real frame generation needs to tell the compositor exactly when each
    // extra buffer is meant to appear, or it just races them through and
    // flickers. Resolve that extension; if it's missing on this device,
    // fall back to a single-swap blend (no frame doubling, but safe).
    void *presTimeSym =
        reinterpret_cast<void *>(eglGetProcAddress("eglPresentationTimeANDROID"));
    mPresentationTimeFn =
    ...
    const bool moduleRegistered =
        pl::modmenu::ModuleBuilder(kModuleId, "Frame Generator")
            .modId(getSelf().getId())
            .description(
                "Attempts real frame generation (extra interpolated "
                "frames, properly timestamped for the display) where the "
                "device supports it; otherwise falls back to a safe "
                "blend-only smoothing mode. Set strength to 0 to disable "
                "the effect entirely.")
            .defaultEnabled(true)
...
private:
  ...
  EGLBoolean handleSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    if (mHaveHistory && mTrueFrameGenAvailable) {
      // Real frame generation: present an interpolated frame timestamped
      // to land halfway between the last real frame and this one, then
      // present the real frame at its normal cadence point. Explicit
      // presentation timestamps are what stop this from racing
      // SurfaceFlinger the way an un-timed double swap did.
      int64_t intervalNs = nowNs - mLastCallTimeNs;
      // Guard against loading screens / pauses producing a huge or
      // negative "interval" that would schedule frames nonsensically.
      intervalNs = std::clamp<int64_t>(intervalNs, 2'000'000LL, 100'000'000LL);

      const int64_t interpTimeNs = mLastRealPresentTimeNs + intervalNs / 2;
      ...
      mPresentationTimeFn(dpy, surface, interpTimeNs);
      drawBlended(mix);
    ...
    if (mHaveHistory) {
      // No presentation-time support on this device: blend into the one
      // frame we're about to present rather than risking an un-timed
      // extra swap.
      const float mix =
          static_cast<float>(std::clamp(mBlendPercent.load(), 0, 100)) /
          100.0f;
  ...
  void drawBlended(float mix) {
    ...
    // Critical: kQuadVerts is a plain CPU pointer. If a VBO is still
    // bound here (very likely — the game almost certainly uses one),
    // glVertexAttribPointer would reinterpret that pointer as a byte
    // OFFSET into the bound VBO instead of a real address, reading
    // garbage and producing exactly the "wrong shapes/colors" corruption.
